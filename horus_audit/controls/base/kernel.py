...
def check_sysctl_value(
    ...
    *,
    ...
    if result.code != 0:
        return ControlResult.skipped_(control_name, f"Cannot read sysctl {key}")

    sysctl_value = result.stdout.strip()

    if str(expected_value) == sysctl_value:
        return ControlResult.passed_(control_name, f"{key}={sysctl_value}")

    return ControlResult.failed_(control_name, f"{key}={sysctl_value}, expected {expected_value}")
